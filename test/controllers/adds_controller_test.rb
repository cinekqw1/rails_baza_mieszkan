...
class AddsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create add" do
    assert_difference('Add.count') do
      post adds_url, params: { add: { address: @add.address, description: @add.description, price: @add.price, title: @add.title } }
    end
  ...
  test "should update add" do
    patch add_url(@add), params: { add: { address: @add.address, description: @add.description, price: @add.price, title: @add.title } }
